...
conn = MySQLdb.connect (host = "localhost",
                        ...
                        db = "connor")
cursor = conn.cursor()
creatoruname = form.getvalue('creatoruname')
creatoruname.strip("()\'\;")
cursor.execute("SELECT * FROM pictures right join users on pictures.creatorid = users.id WHERE users.uname = \"%s\"" % creatoruname)
...
if (len(rows) > 0):
	...
	print '<head>'
	print '<link rel="stylesheet" type="text/css" href="http://cs.coloradocollege.edu:/~cp344/cr/tostr/styles/main.css">'
	print '<p><h3>Creator not found.</h3><p>'
	print '</head>'
	print'</html>'
